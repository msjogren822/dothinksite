...
type StyleKey = 'cartoon' | 'bauhaus' | 'impressionist' | 'cubist' | 'art_deco' | 'watercolor' | 'oil_painting' | 'cyberpunk' | 'minimalist' | 'vintage_poster';

const STYLE_MAP: Record<StyleKey, { label: string; render: string }> = {
  cartoon: {
    label: 'cartoon',
...
async function callGeminiImagePreview(genAI: GoogleGenerativeAI, parts: any[]) {
  ...
  if (candidates && candidates.length > 0) {
    ...
    if (candidate.content && candidate.content.parts) {
      ...
      if (imagePart?.inlineData?.data) {
        return imagePart.inlineData.data as string; // base64 (no data URL prefix)
      }
    }
  }
  // Fallback to text if no image part found
  const text = (response as any).text?.() ?? 'Model did not return an image.';
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const {
      imageData, // background base64 (no prefix expected from client, but handle both)
      mimeType,
      foregroundImageData,
      foregroundMimeType,
      styleKey = 'cartoon',
      ...
      imageData: string; mimeType?: string; foregroundImageData: string; foregroundMimeType?: string;
      styleKey?: StyleKey; enforceStyle?: boolean; promptExtras?: string;
    };
    ...
    const style = STYLE_MAP[(styleKey as StyleKey) in STYLE_MAP ? (styleKey as StyleKey) : 'cartoon'];
